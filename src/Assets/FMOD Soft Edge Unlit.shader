...
Shader "Transparent/Cutout/FMOD Soft Edge Unlit" {
Properties {
	_Color ("Main Color", Color) = (1, 1, 1, 0)
	_MainTex ("Base (RGB) Alpha (A)", 2D) = "white" {}
	_Cutoff ("Base Alpha cutoff", Range (0,.9)) = 0.001
	_fmod ("FuckMod", float) = 0.5
}
	...
	Pass {  
		CGPROGRAM
			...
			half4 frag (v2f i) : COLOR
			{
				...
				half4 col = _Color * tex2D(_MainTex, i.texcoord + fmod(i.texcoord,_fmod*r));
				col.rgb = col.rgb+dot(col.rgb,fmod(col.rgb,_fmod));
				clip( fmod( col.r - _Cutoff,_fmod) );
				return col;
			}
		ENDCG
	}
